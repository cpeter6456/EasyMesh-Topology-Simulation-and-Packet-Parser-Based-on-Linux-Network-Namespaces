...
{
    fprintf(stderr, "usage: %s [--rssi-link <agent1|agent2> <dbm>] [--rssi-link-file <agent1|agent2> <path>]\n", prog);
}
...
static int parse_rssi(const char *text, int *value)
{
    ...
    if (text == end || *end != '\0' || parsed < -127 || parsed > 127) return -1;
    *value = (int)parsed;
    return 0;
}
...
static int link_index_for_name(const struct node_config *node, const char *name)
{
    ...
    if (strcmp(name, "agent1") == 0) last_byte = 2;
